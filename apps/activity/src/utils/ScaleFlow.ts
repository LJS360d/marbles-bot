...
export class ScaleFlow {
	...
	static getX(x: number | string): number {
		...
		const gameWidth = ScaleFlow.scaleManager.width;

		const percentNum = Number.parseInt(x, 10);
	...
	static getY(y: number | string): number {
		...
		const gameHeight = ScaleFlow.scaleManager.height;

		const percentNum = Number.parseInt(y, 10);
